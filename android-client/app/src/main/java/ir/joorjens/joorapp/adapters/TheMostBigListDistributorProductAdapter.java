...
public class TheMostBigListDistributorProductAdapter extends RecyclerView.Adapter<TheMostBigListDistributorProductAdapter.TheMostBigItemHolder> {
    ...
    public TheMostBigListDistributorProductAdapter(Context context, List<DistributorProduct> items, int colorId) {
        mContext = context;
        mItems = items;
    ...
    public void onBindViewHolder(TheMostBigItemHolder holder, int position) {
        ...
        holder.mTvText1.setText(item.getProductName() + " " + item.getProductBrandTypeName());
        ...
        holder.mTvText1_1.setVisibility(View.VISIBLE);
        holder.mTvText1_1.setText(allDetails);

        if(mColorId == R.color.cheap_color){
            holder.mTvText3.setVisibility(View.VISIBLE);
            holder.mTvText3.setText(String.format("%,d", item.getPriceMin()) + " تومان");
        }

        StaticHelperFunctions.loadImage(mContext, item.getProductImage(), holder.mIvThumbnail.getImageView());

        holder.mRlMainBorder.setTag(item.getProductId());
        holder.mRlMainBorder.setOnClickListener(onShowDProduct);
    ...
    private View.OnClickListener onShowDProduct = new View.OnClickListener() {
        @Override
