...
@Entity
@Table(name = "distributor_deliverer")
public class DistributorDeliverer extends AbstractModel implements CustomerInterface {
    private static final long serialVersionUID = 1395L;

    //------------------------------------------------
    public static String getEN() {
        return "تحویل‌دهنده شرکت پخش";
    }
    ...
    @JoinColumn(name = "id", foreignKey = @ForeignKey(name = "FK_DISTRIBUTOR_DELIVERER__CUSTOMER__customer"))
    @JsonIgnore
    private Customer customer;

    @OneToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "id_distributor", foreignKey = @ForeignKey(name = "FK_DISTRIBUTOR_DELIVERER__DISTRIBUTOR__distributor"))
    @JsonIgnore
    private Distributor distributor;

    @OneToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "id_vehicle", foreignKey = @ForeignKey(name = "FK_DISTRIBUTOR_DELIVERER__VEHICLE__vehicle"))
    @JsonIgnore
    private Vehicle vehicle;

    @OneToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "id_last_login", foreignKey = @ForeignKey(name = "FK_DISTRIBUTOR_DELIVERER__CUSTOMER_LOGIN__lastLogin"))
    @JsonIgnore
    private CustomerLogin lastLogin;
    //------------------------------------------------

    public DistributorDeliverer() {
    }

    public DistributorDeliverer(long id) {
        this.id = id;
    }
    ...
    public long getVehicleId() {
        return (this.vehicle != null) ? this.vehicle.getId() : 0;
    }

    @JsonGetter
    public String getVehicleBrandTypeName() {
        return (this.vehicle != null) ? this.vehicle.getVehicleBrandTypeName() : null;
    }
